...
public class WalletAPIClient {
    ...
    public static var `default` = WalletAPIClient(baseURL: "https://api.pace.cloud/wallet")

    /// A list of RequestBehaviours that can be used to monitor and alter all requests
    public var behaviours: [WalletAPIRequestBehaviour] = [WalletAPIRequestBehaviourImplementation()]
    ...
    public var decodingQueue = DispatchQueue(label: "WalletAPIClient", qos: .utility, attributes: .concurrent)
    ...
    public init(baseURL: String, configuration: URLSessionConfiguration = .default, defaultHeaders: [String: String] = [:], behaviours: [WalletAPIRequestBehaviour] = []) {
        self.baseURL = baseURL
        self.behaviours = self.behaviours + behaviours
        self.defaultHeaders = defaultHeaders
        jsonDecoder.jsonDecoder.dateDecodingStrategy = .custom(dateDecoder)
        jsonEncoder.dateEncodingStrategy = .formatted(WalletAPI.dateEncodingFormatter)
    ...
    ///   - complete: A closure that gets passed the WalletAPIResponse
    /// - Returns: A cancellable request. Note that cancellation will only work after any validation RequestBehaviours have run
    @discardableResult
    public func makeRequest<T>(_ request: WalletAPIRequest<T>,
                               behaviours: [WalletAPIRequestBehaviour] = [],
                               currentUnauthorizedRetryCount: Int = 0,
                               currentRetryCount: Int = 0,
                               completionQueue: DispatchQueue = DispatchQueue.main,
                               complete: @escaping (WalletAPIResponse<T>) -> Void) -> CancellableWalletAPIRequest? {
        // create composite behaviour to make it easy to call functions on array of behaviours
        let requestBehaviour = WalletAPIRequestBehaviourGroup(request: request, behaviours: self.behaviours + behaviours)
        ...
        do {
            ...
            let response = WalletAPIResponse<T>(request: request, result: .failure(error))
            complete(response)
            return nil
        }
        ...
        let cancellableRequest = CancellableWalletAPIRequest(request: request.asAny())
        ...
        requestBehaviour.validate(urlRequest) { result in
            ...
            case .failure(let error):
                ...
                let response = WalletAPIResponse<T>(request: request, result: .failure(error), urlRequest: urlRequest)
    ...
    private func makeNetworkRequest<T>(request: WalletAPIRequest<T>,
                                       urlRequest: URLRequest,
                                       cancellableRequest: CancellableWalletAPIRequest,
                                       requestBehaviour: WalletAPIRequestBehaviourGroup,
                                       currentUnauthorizedRetryCount: Int,
                                       currentRetryCount: Int,
                                       completionQueue: DispatchQueue,
                                       complete: @escaping (WalletAPIResponse<T>) -> Void) {
    ...
    private func performRequest<T>(request: WalletAPIRequest<T>,
                                   urlRequest: URLRequest,
                                   cancellableRequest: CancellableWalletAPIRequest,
                                   requestBehaviour: WalletAPIRequestBehaviourGroup,
                                   currentUnauthorizedRetryCount: Int,
                                   currentRetryCount: Int,
                                   completionQueue: DispatchQueue,
                                   complete: @escaping (WalletAPIResponse<T>) -> Void) -> URLSessionDataTask {
        ...
        return session.dataTask(with: urlRequest, completionHandler: { [weak self] data, response, error -> Void in
            ...
            self?.decodingQueue.async {
                ...
                if API.shouldRetryRequest(currentRetryCount: newRetryCount,
                    ...
                    let response = WalletAPIResponse<T>(request: request, result: result, urlRequest: urlRequest)
    ...
    private func handleResponse<T>(request: WalletAPIRequest<T>,
                                   requestBehaviour: WalletAPIRequestBehaviourGroup,
                                   data: Data?,
        ...
        if let error = error {
            ...
            let response = WalletAPIResponse<T>(request: request, result: result, urlRequest: urlRequest, urlResponse: response, data: data)
            requestBehaviour.onResponse(response: response.asAny())
    ...
    private func handleResponseData<T>(request: WalletAPIRequest<T>,
                                       requestBehaviour: WalletAPIRequestBehaviourGroup,
                                       data: Data?,
                                       response: HTTPURLResponse,
                                       urlRequest: URLRequest,
        ...
        guard let data = data else {
            ...
            let response = WalletAPIResponse<T>(request: request, result: result, urlRequest: urlRequest, urlResponse: response, data: nil)
            requestBehaviour.onResponse(response: response.asAny())
...
public extension WalletAPIClient {
    ...
    func makeRequest<T>(_ request: WalletAPIRequest<T>,
                        behaviours: [WalletAPIRequestBehaviour] = [],
                        currentUnauthorizedRetryCount: Int = 0,
                        currentRetryCount: Int = 0) async -> WalletAPIResponse<T> {
