...
public protocol WalletAPIRequestBehaviour {
    ...
    func onFailure(request: AnyWalletAPIRequest, urlRequest: URLRequest?, response: HTTPURLResponse, error: APIClientError)
...
public extension WalletAPIRequestBehaviour {
    func modifyRequest(request: AnyWalletAPIRequest, urlRequest: URLRequest) -> URLRequest { return urlRequest }
    func validate(request: AnyWalletAPIRequest, urlRequest: URLRequest, complete: @escaping (RequestValidationResult) -> Void) {
...
struct WalletAPIRequestBehaviourImplementation: WalletAPIRequestBehaviour {
    func onFailure(request: AnyWalletAPIRequest, urlRequest: URLRequest?, response: HTTPURLResponse, error: APIClientError) {
        ...
        case .networkError(let error) where (error as NSError).code == NSURLErrorCancelled:
            SDKLogger.d("[WalletAPI] Request with url (\(url)) was canceled.")

        default:
            let requestId: String = response.allHeaderFields["request-id"] as? String ?? "unknown"
            SDKLogger.e("[WalletAPI] Request (\(url)) with request-id: \(requestId) failed with error: \(error.description)")
        }
    }
}

// Group different RequestBehaviours together
struct WalletAPIRequestBehaviourGroup {

    let request: AnyWalletAPIRequest
    let behaviours: [WalletAPIRequestBehaviour]

    init<T>(request: WalletAPIRequest<T>, behaviours: [WalletAPIRequestBehaviour]) {
    ...
    func onResponse(response: AnyWalletAPIResponse) {
        behaviours.forEach {
...
extension WalletAPIService {
    public func asAny() -> WalletAPIService<AnyResponseValue> {
        return WalletAPIService<AnyResponseValue>(id: id, tag: tag, method: method, path: path, hasBody: hasBody, securityRequirements: securityRequirements)
    }
}
