...
public typealias AnyFuelingAPIResponse = FuelingAPIResponse<AnyResponseValue>

public struct FuelingAPIResponse<T: APIResponseValue> {
    ...
    init(request: FuelingAPIRequest<T>, result: APIResult<T>, urlRequest: URLRequest? = nil, urlResponse: HTTPURLResponse? = nil, data: Data? = nil) {
...
extension FuelingAPIResponse {
    public func asAny() -> FuelingAPIResponse<AnyResponseValue> {
        return FuelingAPIResponse<AnyResponseValue>(request: request.asAny(), result: result.map{ $0.asAny() }, urlRequest: urlRequest, urlResponse: urlResponse, data: data)
    }
}
