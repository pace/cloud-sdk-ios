//
// Generated by SwagGen
// https://github.com/pace/SwagGen
//

import Foundation

public class PCReferenceStatus: APIModel {

    /** Type */
    public enum PCType: String, Codable, Equatable, CaseIterable {
        case referenceStatus = "referenceStatus"
    }

    public var attributes: Attributes?

    /** Service Provider PRN */
    public var id: String?

    /** Type */
    public var type: PCType?

    public class Attributes: APIModel {

        /** Availability status of the referenced resource */
        public enum PCStatus: String, Codable, Equatable, CaseIterable {
            case online = "online"
            case offline = "offline"
        }

        /** Availability status of the referenced resource */
        public var status: PCStatus?

        /** Time of status last update (iso8601) */
        public var updatedAt: DateTime?

        public init(status: PCStatus? = nil, updatedAt: DateTime? = nil) {
            self.status = status
            self.updatedAt = updatedAt
        }

        public required init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: StringCodingKey.self)

            status = try container.decodeIfPresent("status")
            updatedAt = try container.decodeIfPresent("updatedAt")
        }

        public func encode(to encoder: Encoder) throws {
            var container = encoder.container(keyedBy: StringCodingKey.self)

            try container.encodeIfPresent(status, forKey: "status")
            try container.encodeIfPresent(updatedAt, forKey: "updatedAt")
        }

        public func isEqual(to object: Any?) -> Bool {
          guard let object = object as? Attributes else { return false }
          guard self.status == object.status else { return false }
          guard self.updatedAt == object.updatedAt else { return false }
          return true
        }

        public static func == (lhs: Attributes, rhs: Attributes) -> Bool {
            return lhs.isEqual(to: rhs)
        }
    }

    public init(attributes: Attributes? = nil, id: String? = nil, type: PCType? = nil) {
        self.attributes = attributes
        self.id = id
        self.type = type
    }

    public required init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: StringCodingKey.self)

        attributes = try container.decodeIfPresent("attributes")
        id = try container.decodeIfPresent("id")
        type = try container.decodeIfPresent("type")
    }

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: StringCodingKey.self)

        try container.encodeIfPresent(attributes, forKey: "attributes")
        try container.encodeIfPresent(id, forKey: "id")
        try container.encodeIfPresent(type, forKey: "type")
    }

    public func isEqual(to object: Any?) -> Bool {
      guard let object = object as? PCReferenceStatus else { return false }
      guard self.attributes == object.attributes else { return false }
      guard self.id == object.id else { return false }
      guard self.type == object.type else { return false }
      return true
    }

    public static func == (lhs: PCReferenceStatus, rhs: PCReferenceStatus) -> Bool {
        return lhs.isEqual(to: rhs)
    }
}
