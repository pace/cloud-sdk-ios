...
public class GeoJSONAPIClient {
    ...
    public var behaviours: [GeoJSONAPIRequestBehaviour] = [GeoJSONAPIRequestBehaviourImplementation()]
    ...
    @discardableResult
    public func makeRequest<T>(_ request: GeoJSONAPIRequest<T>, behaviours: [GeoJSONAPIRequestBehaviour] = [], currentNumberOfRetries: Int = 0, completionQueue: DispatchQueue = DispatchQueue.main, complete: @escaping (GeoJSONAPIResponse<T>) -> Void) -> CancellableGeoJSONAPIRequest? {
    ...
    private func makeNetworkRequest<T>(request: GeoJSONAPIRequest<T>, urlRequest: URLRequest, cancellableRequest: CancellableGeoJSONAPIRequest, requestBehaviour: GeoJSONAPIRequestBehaviourGroup, currentNumberOfRetries: Int, completionQueue: DispatchQueue, complete: @escaping (GeoJSONAPIResponse<T>) -> Void) {
    ...
    private func handleResponseData<T>(request: GeoJSONAPIRequest<T>,
                                       requestBehaviour: GeoJSONAPIRequestBehaviourGroup,
                                       data: Data?,
                                       response: HTTPURLResponse,
                                       urlRequest: URLRequest,
        ...
        guard let data = data else {
            ...
            let response = GeoJSONAPIResponse<T>(request: request, result: result, urlRequest: urlRequest, urlResponse: response, data: nil)
            requestBehaviour.onResponse(response: response.asAny())
